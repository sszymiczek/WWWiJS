    ...
    if (isset($_POST['dislike-button'])) {
        try {
            ...
            if ($like = $stmt->fetch(PDO::FETCH_ASSOC)) {
                if ($like['is_positive'] == 1) {
                    $stmt2 = $dbh->prepare("UPDATE likes SET is_positive = 0 WHERE fpfti_id = :fpfti_id");
                ...
                $stmt2->execute([':user_id' => $_SESSION['id'], ':fpfti_id' => $_POST['fpfti-id'], ':is_positive' => 0]);
